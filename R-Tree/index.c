...
//
int RTreeSearch(struct RNode *N, struct Rect *R, SearchHitCallback shcb, void* cbarg)
{
...
static int RTreeInsertRect2(struct Rect *r,
		int tid, struct RNode *n, struct RNode **new_node, int level)
{
/*
	register struct Rect *r = R;
	register int tid = Tid;
	register struct RNode *n = N, **new_node = New_node;
...
//
int RTreeInsertRect(struct Rect *R, int Tid, struct RNode **Root, int Level)
{
	register struct Rect *r = R;
	register int tid = Tid;
	register struct RNode **root = Root;
	register int level = Level;
	register int i;
	register struct RNode *newroot;
	struct RNode *newnode;
	struct Branch b;
	int result;
...
//
static void RTreeReInsert(struct RNode *n, struct ListNode **ee)
{
...
//
static int
RTreeDeleteRect2(struct Rect *R, int Tid, struct RNode *N, struct ListNode **Ee)
{
	register struct Rect *r = R;
	register int tid = Tid;
	register struct RNode *n = N;
	...
	else  // a leaf node
	{
		...
		{
			if (n->branch[i].child &&
			    n->branch[i].child == (struct RNode *) tid)
			{
				RTreeDisconnectBranch(n, i);
				return 0;
			}
...
//
int RTreeDeleteRect(struct Rect *R, int Tid, struct RNode**Nn)
{
	register struct Rect *r = R;
	register int tid = Tid;
	register struct RNode **nn = Nn;
	register int i;
	register struct RNode *tmp_nptr;
